...
# Additions (-2 -1 0 1 2 questions)
# X[""] = df_old[""]
df_old = pd.read_csv("preprocessresult.csv", header=0)
df_old["id"] = df_old.index + 1     # R 需要 id，但是 python 不用
X["youbike_easy"] = df_old["youbike_easy"]

X["bike_flex"] = df_old["bike_flex"]
X["bike_no_leisure"] = df_old["bike_no_leisure"]

print(X.columns)

#In[3]
X_train, X_test, y_train, y_test = train_test_split(X, y_bus, test_size=0.2, random_state=0)
'''
print(X_train)
print(y_train)
print(X_test)
